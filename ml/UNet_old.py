    ...
    with torch.no_grad():
        ...
        for gid in gid_gnode_dict.keys():
            ...
            for p in points:
                x = p[1] #+ x_range[0]
                y = p[0] #+ y_range[0]
                arc_predictions += [int(pred[0] > pred_thresh) for pred in __get_prediction_correctness(segmentation,
                                                                               logits_predicted,(x,y))]
                arc_segmentation_logits += [int(pred[1] > pred_thresh) for pred in __get_prediction_correctness(segmentation,
                                                                               logits_predicted,(x,y))]
...
class dataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.with_range:
            sample = [torch.as_tensor(self.data_array[index][0],dtype=torch.float),
                      torch.as_tensor(self.data_array[index][1],dtype=torch.float),
                      torch.as_tensor(self.data_array[index][2],dtype=torch.int)]
        else:
            sample = [torch.as_tensor(self.data_array[index][0], dtype=torch.float),
                      torch.as_tensor(self.data_array[index][1], dtype=torch.float)]
...
class UNetwork( MLGraph, nnModule, object):
    ...
    #     for y_rng, x_rng in range_group:
    #         print('y_range', y_rng)
    #         print('x_range', x_rng)
    #
    ...
    #         segmentation = self.generate_pixel_labeling((x_rng, y_rng),
    #                                                seg_image=seg)
    #         # segmentation = segmentation[y_rng[0]:y_rng[1], x_rng[0]:x_rng[1]]
    #
    #         print("segmentation shape", segmentation.shape)
    #         dataset.append((image, segmentation, region_contour))
    #
    #     return dataset[1]
    #
    ...
    #
    #     print(train_im.shape, 'train im shape')
    #     return train_im.astype(np.uint8)




    def get_data_crops(self, image=None, x_range=None,y_range=None, with_range=True,
                       dataset=[], resize=False, full_img=False):
        ...
        for y_rng, x_rng in range_group:
            bounds = (x_rng,y_rng) if not full_img else ([0,self.image.shape[0]],
            ...
            seg = self.generate_pixel_labeling( bounds,
                                                         seg_image=seg)
            #segmentation = segmentation[y_rng[0]:y_rng[1], x_rng[0]:x_rng[1]]
            if with_range:
                dataset.append((train_im_crop, seg, (x_rng, y_rng)))
            else:
                dataset.append((train_im_crop, seg))
    ...
    def expansion_block(self, in_channels, mid_channel, out_channels, kernel_size=3, padding=2):#True):
        #if padding:
    ...
    def final_block(self, in_channels, mid_channel, out_channels, kernel_size=3, padding=2):#True):#
        block = torch.nn.Sequential(
            torch.nn.Conv2d(kernel_size=kernel_size, in_channels=in_channels,
                            out_channels=mid_channel, padding=int(padding)),
            torch.nn.ELU(),
            ...
            torch.nn.Conv2d(kernel_size=1, in_channels=mid_channel,
                            out_channels=out_channels, padding=padding),#, padding=padding-1),
            torch.nn.ELU(),
            torch.nn.BatchNorm2d(out_channels),
        )
        return block
    ...
    def __init__(self,in_channel, out_channel, skip_connect=True, kernnel_size=2,
        ...
        self.reverse_direction = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(kernel_size=3, in_channels=self.init_expansion * (self.multiplier**2),
                            out_channels=self.base_channel_dim, padding=0),
            torch.nn.ReLU(),
            ...
            torch.nn.ConvTranspose2d(in_channels=self.base_channel_dim,
                                     ...
                                     kernel_size=2, stride=2,
                                     padding=0 , output_padding=0)#self.expansion_out_padding )
        )  # 1024
        #self.multiplier *= 2
    ...
    def crop_and_concat(self, upsampled, bypass, crop=True):#True):#True):#True):#True):
    ...
    def forward(self, x):
        ...
        up_block2 = cat_layer2 if not self.skip_connect else self.crop_and_concat(cat_layer2,
                                                                                  down_block1)
    ...
    def collect_boxes(self, region_list, number_samples=None, resize=False, run_num=-1, training_set=False):
        ...
        for current_box_idx in number_boxes:
            ...
            training_set_boxes, test_and_val_set = self.box_select_geomsc_training(x_range=X_BOX,
                                                                                  y_range=Y_BOX)
            ...
            for i, t_class in enumerate(training_set_boxes):
            ...
            test_dataset = self.get_data_crops(self.image, x_range=X_BOX, with_range = not training_set,
                                                    y_range=Y_BOX, dataset=test_dataset, resize=resize)
            #self.see_image(gt_seg=test_dataset[0][1], save=False)


        inf_or_train_dataset = dataset(test_dataset, do_transform= training_set,
    ...
    def infer(self, running_best_model, dataset=None, training_window_file=None,
              load=False, view_results=False, verbose=False,
        ...
        total_inf_img = np.zeros((self.X , self.Y))#.astype(np.float32)
        total_gt_seg = np.zeros((self.X , self.Y)).astype(np.uint8)
        ...
        pred_tile = np.zeros([1, 1, se.shape[0], se.shape[1]])#.astype(np.float32)
        ...
        with torch.no_grad():
            ...
            for image, segmentation, ranges in test_loader:
                ...
                num_val += 1
                ranges = ranges.cpu().detach().numpy()
                ranges = np.array(ranges).astype(np.float32)
                ranges = [int(r) for r in ranges.flatten()]

                print("ranges numpy", ranges, image.shape)

                image, segmentation = image.to(device), segmentation.to(device)
                image = image.unsqueeze(1)

                im_cpu = image.cpu().detach().numpy()
                plt.figure()
                plt.title("Input Image")
                plt.imshow(im_cpu[0, 0, :, :])
                plt.show()

                seg_cpu = segmentation.cpu().detach().numpy()
                plt.figure()
                plt.title("Input Image")
                plt.imshow(seg_cpu[0,:,:])
                plt.show()
                ...
                logits_predicted =  predicted.cpu().detach().numpy()#np.concatenate((pred_tile,

                plt.figure()
                plt.title("Input Image")
                plt.imshow(logits_predicted[0,0,:,:])
                plt.show()

                logits_predicted = logits_predicted#.astype(np.float32)
                pred_tile_cpu = deepcopy(logits_predicted)#.astype(np.float32)

                print("    pred_shape, holder shape ", pred_tile_cpu.shape, ' ', pred_tile.shape)
                pred_tile[:,:,:,:] = pred_tile_cpu[:,:,:,:]
                ...
                if view_results:
                    self.see_image(
                        names=("og_tile1" , "groundseg_tile1" ,
                               "pred_tile1"),
                        image_seg_set=(image_subset, ground_truth_seg, pred_tile),
                ...
                total_img[x_range[0]:x_range[1], y_range[0]:y_range[1]] = image_subset
                ...
                if view_results:
                    ...
                    self.see_image(image_seg_set=(image_subset, ground_truth_seg, pred_tile),
                                   names=("og_tile2", "groundseg_tile2",
                                          "pred_tile2"),
                ...
                total_inf_img[x_range[0] + pad: x_range[1] - pad,
                y_range[0] + pad: y_range[1] - pad] = pred_tile[0, 0, pad:pred_tile.shape[2] - pad ,
                                                      pad:pred_tile.shape[3] - pad ]

                out_folder = os.path.join(self.pred_session_run_path,
                                          str(self.training_size))

                print(" ")
                print("    * images writen to: ", out_folder)
                print(" ")

                if not os.path.exists(out_folder):
                    os.makedirs(out_folder)
                self.save_image(image_seg_set=(total_img, total_gt_seg, total_inf_img),
            ...
            for ranges, pred_im in zip(range_list, pred_list):
                ...
                offsetx = int(pred_im.shape[0] // 10)
                if x_range[0] == 0:
                    offsetx = 0
                numx = (x_range[1] - (x_range[0] + offsetx)) - int(pred_im.shape[0] // 10)
                if x_range[1] == pred_im.shape[0]:
                    numx = (x_range[1] - (x_range[0] + offsetx))
                offsety = int(pred_im.shape[1] //10)
                if y_range[0] == 0:
                    offsety = 0
                numy = (y_range[1] - (y_range[0] + offsety)) - int(pred_im.shape[1]//10)
                if y_range[1] == pred_im.shape[1]:
                    numy = (y_range[1] - (y_range[0] + offsety))


                # total_inf_img[x_range[0]+pad : x_range[1]-pad,
                # y_range[0]+pad : y_range[1]-pad] = pred_im[0,0,pad:pred_im.shape[0]-pad-1,
                #                                               pad:pred_im.shape[1]-pad-1]

            # out_folder = os.path.join(self.pred_session_run_path,
            #                           str(self.training_size))
            #
            # print(" ")
            # print("    * results writen to: ", out_folder)
            # print(" ")
            #
            # if not os.path.exists(out_folder):
            #     os.makedirs(out_folder)
        ...
        F1_score_img, labels_img, predictions_img = 0,0,0
        # get_image_prediction_score(predicted=total_inf_img[None][None],
        ...
        return test_losses, val_imgs, val_segs, val_img_preds, running_val_loss, F1_score_img,\
               labels_img, predictions_img
...
class UNet_Trainer:
    def __init__(self, UNet : UNetwork, train_dataset=None, val_dataset=None,
        ...
        self.batch_size = self.UNet.params['batch_size']
    ...
    def launch_training(self, view_results=False,pred_thresh=0.5):
        ...
        for epoch in range(n_epochs):
            ...
            for image, segmentation in train_loader:
                ...
                #image  = image[:,ranges[0]:ranges[1], ranges[2]:ranges[3] ]
                #segmentation = segmentation[:,ranges[0]:ranges[1], ranges[2]:ranges[3] ]
                ...
                train_loss = criterion(predicted, segmentation)

                predicted = predicted.to(torch.float32)#uint8)
                segmentation = segmentation.to(torch.uint8)
                ...
                self.predicted_segmentation = self.predicted_segmentation.astype(np.float32)#uint8)
                self.ground_truth_seg = self.ground_truth_seg.astype(np.uint8)
                ...
                if steps % print_every == 0:
                    ...
                    if False:
                        with torch.no_grad():
                            for image, segmentation, range_val in val_dataloaders:
                                range_val = range_val.cpu().detach().numpy()
                                range_val = np.array(range_val).astype(np.float32)
                                range_val = [int(r) for r in range_val.flatten()]

                                print("ranges numpy", range_val, image.shape)

                                image = image[:, range_val[0]:range_val[1], range_val[2]:range_val[3]]
                                segmentation = segmentation[:,range_val[0]:range_val[1], range_val[2]:range_val[3]]
                                image, segmentation = image.to(self.device), segmentation.to(self.device)
                    ...
                    print("Training F1 image: {acc}".format(acc=F1_score_img))
                    ...
                    if F1_score_topo > best_f1:  # and val_score > 0.35:
                        ...
                        self.running_best_model = copy.deepcopy(self.UNet)
        ...
        if view_results:
            with torch.no_grad():
                self.UNet.see_image(image_seg_set=(total_img, total_gt_seg, total_inf_img),
                               names=("total_og_tiled", "total_groundseg", "total_prediction"),
                               as_grey=True,
                               dirpath=out_folder,
                               save=True)
        ...
        with torch.no_grad():
            torch.cuda.empty_cache()
        return train_losses, test_losses, F1_scores, best_f1, val_imgs, val_segs,\
               sample_losses, val_img_preds, self.running_best_model
